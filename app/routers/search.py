...
CARD_COLS = ["racquet_name", "racquet_url", "racquet_img", "racquet_rating", "racquet_rating_count", "racquet_price", "racquet_description"]
TOP_N = 20
...
async def search(request: Request,
    ...
    for rank, (racquet_id, score) in enumerate(result.fused_result[0:TOP_N], start = 1):
        ...
        racquet_card = RacquetCard(racquet_id=racquet_id, racquet_rank=rank,
                                   racquet_name=racquet["racquet_name"], racquet_url=racquet["racquet_url"],
                                   racquet_img=racquet["racquet_img"], racquet_rating=racquet["racquet_rating"],
                                   racquet_rating_count=racquet["racquet_rating_count"], racquet_price=racquet["racquet_price"],
                                   racquet_description=racquet["racquet_description"], racquet_match_score=racquet_match_score)
        cards.append(racquet_card)
        search_results_rows.append((result_id, search_id, racquet_id, rank, None))

    
    # Write to searches
    db.execute("INSERT INTO searches VALUES (%s, %s, %s, %s, %s, %s)", 
           (search_id, raw_query, parsing_status, semantic_query, keyword_query, now))
    
    # Write to impressions as a batch of 20
    psycopg2.extras.execute_values(db, 
                                  "INSERT INTO search_results (result_id, search_id, racquet_id, rank, liked) VALUES %s",
                                  search_results_rows)
    
    return SearchResponse(search_id=search_id, parsing_status=result.parsing_status, results=cards)