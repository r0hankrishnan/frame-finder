...
async def feedback(
    ...
    db.execute(
        "UPDATE search_results SET liked = %s WHERE search_id = %s AND racquet_id = %s",
        (liked, body.search_id, body.racquet_id),
    )

    if db.rowcount == 0:
        logger.warning(
            f"Feedback update matched no rows: search_id={body.search_id} racquet_id={body.racquet_id}"
        )
