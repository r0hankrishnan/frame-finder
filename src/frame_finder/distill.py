"""Module to create prompts for and orchestrate racquet description distillation."""
...
class DistilledDescriptionItem(BaseModel):
    ...
    distilled_description: str = Field(
        description="The concise, feel-focused rewritten racquet description."
    )


class DistilledDescriptionBatch(BaseModel):
    descriptions: list[DistilledDescriptionItem] = Field(
        description="An object containing a racquet_id and its corresponding rewritten description."
    )


def build_batch_prompts(
    racquets_df: pd.DataFrame, batch_size: int = 10
    ...
    for start in range(0, len(racquets_df), batch_size):
        chunk = racquets_df.iloc[start : start + batch_size][
            ["racquet_id", "racquet_description"]
        ]
...
def distill_descriptions(
    racquets_df: pd.DataFrame,
    llm_adapter: LLMAdapter,
    output_format: type[DistilledDescriptionBatch],
    partial_save_path: Path,
    batch_size: int = 10,
) -> list[DistilledDescriptionItem]:
    ...
    for batch_num, (prompt, expected_ids) in enumerate(batched_groups, start=1):
        ...
        for i in range(0, 3):  # Tries 3 times, breaks if fails 3 times
            try:
                distilled_batch = llm_adapter.complete(
                    prompt=prompt, output_format=output_format
                )  # Get distilled descs
                distilled_descs = distilled_batch.descriptions

                returned_ids = set(
                    item.racquet_id for item in distilled_descs
                )  # Check length + racquet_ids

                if expected_ids != returned_ids or len(expected_ids) != len(
                    distilled_descs
                ):
                ...
                if i < 2:
                    ...
                    raise Exception(
                        f"Pipeline failed on batch {batch_num}. Partial results saved to {partial_save_path}."
                    )
