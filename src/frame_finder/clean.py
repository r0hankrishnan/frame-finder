"""Module to remove duplicate columns, fix data types, and clean raw racquet data from web scraping."""
...
def extract_numeric_specs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["racquet_head_size_sq_in"] = (
        out["head_size"].str.extract(r"(\d+\.?\d*)\s*(?:in²|in|sq\s*in)").astype(float)
    )
    ...
    out["racquet_length_in"] = (
        out["length"].str.extract(r"(\d+\.?\d*)\s*(?:in²|in|sq\s*in)").astype(float)
    )
    ...
    out["racquet_strung_weight_oz"] = (
        out["strung_weight"].str.extract(r"(\d+\.?\d*)\s*").astype(float)
    )
    ...
    out["racquet_balance_in"] = (
        out["balance"].str.extract(r"(\d+(?:\.\d+)?)\s*in\b").astype(float)
    )

    # Extract Balance number and label separately
    extracted = out["balance"].str.extract(r"(\d+(?:\.\d+)?)\s*(?:pts\s*)?(HL|HH|EB)\b")
    ...
    out["racquet_stiffness"] = out["racquet_stiffness"].replace(
        "N/A (very low)", np.nan
    )
    ...
    out[["racquet_mains", "racquet_crosses"]] = out["string_pattern"].apply(
        extract_mains_crosses
    )
    ...
    out[["racquet_tension_lower", "racquet_tension_upper"]] = out[
        "string_tension"
    ].apply(extract_tension_bounds)
...
def finalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out.rename(
        columns={
            "swingweight": "racquet_swingweight",
